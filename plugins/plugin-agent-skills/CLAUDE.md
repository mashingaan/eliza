...
- **Complete skill inventories.** Prompt metadata and lifecycle lookup operate over every loaded skill. Do not cap `generateSkillsPromptJson`, uninstall/toggle matching, or label an unmarked first-N prefix as the available skill set; use explicit catalog search pagination only where the caller requested it.
