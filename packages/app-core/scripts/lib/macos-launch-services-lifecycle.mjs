...
function runJxa(script, spawnCommand = spawn) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      // error-policy:J1 Translate a failed diagnostic subprocess into the
      // module's explicit inspection/termination result contract.
...
export async function inspectMacApplicationsAtPath(
  ...
  try {
    ...
    // error-policy:J3 Untrusted osascript output is invalid inspection data.
...
export async function requestMacApplicationTermination(
  ...
  try {
    ...
    // error-policy:J3 Invalid authority is rejected before dispatch.
  ...
  try {
    ...
    // error-policy:J3 Untrusted osascript output is invalid termination data.
    return { ok: false, matched: null, error: String(error) };
  }
}
