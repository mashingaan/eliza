...
Story presence is also ratcheted by
`scripts/stories-coverage-baseline.json`. `node scripts/stories-coverage.mjs
--check` fails when either the absolute number of covered components or the
coverage ratio falls below that baseline, or when a component newly appears in
the missing-story set. `audit:story-coverage` remains the report-only command;
`lint:check` runs the regression check.
