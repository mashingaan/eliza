 ...
 * complete-content preservation, and the no-pinned-documents empty case.
 */
...
describe("renderPinnedDocuments", () => {
	...
	it("filters out non-document memories and unpinned documents", () => {
		...
		const result = renderPinnedDocuments(documents);
	...
	it("renders full content without an omission marker", () => {
	...
	it("preserves every byte of long pinned content", () => {
		const documents = [
