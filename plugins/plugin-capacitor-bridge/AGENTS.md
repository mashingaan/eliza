...
- **Interactive-over-background text lane (#11914).** Both TEXT handler paths (bionic UDS delegation and the renderer device bridge) route every generate through the process-wide `InferencePriorityGate` (`@elizaos/core`): interactive turns (the default) dispatch ahead of queued background jobs; requests marked `priority: "background"` run only when the lane is idle, wait at most the RAM-class bound, use that bound as the default when `maxTokens` is omitted, and reject an explicit unsupported output request before decode (`resolveMobileLaneBudget`, driven by the `ELIZA_INFERENCE_RAM_CLASS` env contract from #11760). This is what stops a long autonomous job from self-queueing on the bionic host's resident lock and starving chat without silently shrinking a requested output.
