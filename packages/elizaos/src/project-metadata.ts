 ...
 * create and consumed by upgrade. Writes replace a fully-written temp file so
 * an interrupted upgrade cannot truncate the previous ledger.
 */
...
export function writeProjectMetadata(
  ...
  const tmpPath = path.join(
    metadataDir,
    `.template.json.tmp.${process.pid}.${randomBytes(8).toString("hex")}`,
  );
  try {
    fs.writeFileSync(tmpPath, body);
    fs.renameSync(tmpPath, metadataPath);
  } catch (error) {
    // error-policy:J6 best-effort teardown of the temp file after a failed
    // write or rename; the caller must still see the original failure.
    try {
      fs.rmSync(tmpPath, { force: true });
    } catch {
      // error-policy:J6 temp-file unlink after an already-failing write
    }
    throw error;
  }
}
